...
def generate_heatmap_plot(df):
    ...
    heatmap_data = heatmap_data.fillna(0).mask(heatmap_data == 0, 0)
...
def generate_month_plot(df):
    ...
    for i, year in enumerate(accidents_by_year_month['Year'].unique()):
        ...
        fig.add_trace(go.Bar(
            x=months,
            y=[year_data[year_data['Month'] == month]['Accidents'].values[0] if month in year_data['Month'].values else 0 for month in months],
            name=f'{year}',
    ...
    fig.update_layout(
        ...
        barmode='group',
        xaxis=dict(tickmode='array', tickvals=list(range(0, 12)), ticktext=months),
        **common_layout
    )
...
def generate_location_plot(df):
    ...
    fig.update_traces(name='Barangay')  # Adding name for legend
...
def generate_offense_plot(df):
    ...
    fig = go.Figure(go.Bar(
        ...
        marker_color=colors[:len(top_offenses)],
    ))
    ...
    # Adding horizontal lines
    min_count = top_offenses.min()
    max_count = top_offenses.max()
    line_spacing = (max_count - min_count) / 4  # Adjust the number of lines as needed
    background_lines = [min_count + i * line_spacing for i in range(1, 5)]
...
def generate_victims_age(df):
    ...
    age_groups = ['1-18', '19-30', '31-45', '46-99']
    ...
    fig_victims.add_trace(go.Pie(
        ...
        name='Victims',
        hole=0.3,
        marker=dict(colors=[colors[i % len(colors)] for i in range(len(age_groups))]),
    ))
